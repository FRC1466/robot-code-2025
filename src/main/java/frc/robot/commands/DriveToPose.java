...
public class DriveToPose extends Command {
  ...
  private Supplier<Pose2d> robot = () -> RobotContainer.drivetrain.getState().Pose;

  private Supplier<Translation2d> linearFF = () -> Translation2d.kZero;
  private DoubleSupplier omegaFF = () -> 0.0;

  public DriveToPose(CommandSwerveDrivetrain drive, Supplier<Pose2d> target) {
    this.drive = drive;
    this.target = target;
  ...
  public DriveToPose(
      CommandSwerveDrivetrain drive, Supplier<Pose2d> target, Supplier<Pose2d> robot) {
    this(drive, target);
    this.robot = robot;
  }
  ...
  public void execute() {
    ...
    thetaVelocity =
        MathUtil.interpolate(
            thetaVelocity, omegaFF.getAsDouble() * Units.degreesToRadians(540), thetaS);

    // Command speeds - updated to use the new pattern
    final Translation2d finalDriveVelocity = driveVelocity;
    final double finalThetaVelocity = thetaVelocity;
    // Direct drive style - used in RobotCode2025Public
    drive.runVelocity(
        ChassisSpeeds.fromFieldRelativeSpeeds(
            finalDriveVelocity.getX(),
            finalDriveVelocity.getY(),
            finalThetaVelocity,
  ...
  public void end(boolean interrupted) {
    drive.runVelocity(
        ChassisSpeeds.fromFieldRelativeSpeeds(
            0, 0, 0, RobotContainer.drivetrain.getState().Pose.getRotation()));
